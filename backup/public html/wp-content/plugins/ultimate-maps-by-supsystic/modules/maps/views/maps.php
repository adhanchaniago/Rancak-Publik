...
class mapsViewUms extends viewUms {
		...
		if(!isset($this->_mapsApiUrls[$engine])) {
			...
			switch($engine) {
				case 'bing':
					...
					break;
				case 'leaflet':
					$apiUrl = 'https://unpkg.com/leaflet@1.3.4/dist/leaflet.js';
					break;
				/*case 'mapbox':
	...
	private function _connectApiAssets($engine, $fullEngine) {
		...
		switch($engine) {
			case 'leaflet':
				frameUms::_()->addStyle($assetPrimaryName, 'https://unpkg.com/leaflet@1.3.4/dist/leaflet.css');
				break;
			/*case 'mapbox':
				//frameUms::_()->addStyle('ums_'. $engine. '_maps_api', 'https://api.mapbox.com/mapbox.js/v3.0.1/mapbox.css');
				frameUms::_()->addStyle('ums_'. $engine. '_maps_api', 'https://api.tiles.mapbox.com/mapbox-gl-js/v0.49.0/mapbox-gl.css');
				break;*/
		}
	...
	private function _connectMarkerClusterAssets($engine) {
		switch($engine) {
			case 'leaflet':
				frameUms::_()->addStyle('ums_'. $engine. '_markercluster_api', 'https://cdnjs.cloudflare.com/ajax/libs/leaflet.markercluster/1.4.1/MarkerCluster.css');
				frameUms::_()->addStyle('ums_'. $engine. '_markercluster_def_api', 'https://cdnjs.cloudflare.com/ajax/libs/leaflet.markercluster/1.4.1/MarkerCluster.Default.css');
				frameUms::_()->addScript('ums_'. $engine. '_markercluster_api', 'https://cdnjs.cloudflare.com/ajax/libs/leaflet.markercluster/1.4.1/leaflet.markercluster.js');
				break;
			/*case 'mapbox':
	...
	public function getEditMap($id = 0) {
		...
		$this->assign('icons', frameUms::_()->getModule('icons')->getModel()->getIcons(array('fields' => 'id, path, title, width, height, is_def')));
		...
		switch($engine) {
			case 'l-mapbox':
				$types = array(
					...
					'mapbox.dark' => __('Dark', UMS_LANG_CODE),
					'mapbox.satellite' => __('Satellite', UMS_LANG_CODE),
