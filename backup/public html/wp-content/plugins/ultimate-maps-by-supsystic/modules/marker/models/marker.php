...
class markerModelUms extends modelUms {
	...
	public function save($marker = array(), &$update = false) {
		...
		if(!empty($marker['title'])) {
			...
			if(!isset($marker['icon']) || !frameUms::_()->getModule('icons')->getModel()->iconExists($marker['icon'])) {
				$marker['icon'] = 1;
			}
			...
			if($update) {
				...
				$dbRes = frameUms::_()->getTable('marker')->update($marker, array('id' => $id));

				frameUms::_()->getTable('marker_groups_relation')->delete('marker_id = ' . $marker['id']);
				foreach ($markerGroupIds as $markerId) {
					frameUms::_()->getTable('marker_groups_relation')->insert(array('marker_id'=>$marker['id'], 'groups_id'=>$markerId));
				}
				dispatcherUms::doAction('afterMarkerUpdate', $id, $marker);
			} else {
				dispatcherUms::doAction('beforeMarkerInsert', $marker);
				$dbRes = frameUms::_()->getTable('marker')->insert($marker);
				if($dbRes){
					frameUms::_()->getTable('marker_groups_relation')->delete('marker_id = ' . $dbRes);
					foreach ($markerGroupIds as $markerId) {
						frameUms::_()->getTable('marker_groups_relation')->insert(array('marker_id' => $dbRes, 'groups_id' => $markerId));
					}
				}
				dispatcherUms::doAction('afterMarkerInsert', $dbRes, $marker);
			}
			if($dbRes) {
				if(!$update) {
					$id = $dbRes;
				...
				do_action('ums_save_lang_data', array(
					'type' => 'markers',
					'marker_id' => $id,
					'map' => frameUms::_()->getModule('maps')->getModel()->getMapById($marker['map_id']),
				));
				return $id;
			} else {
	...
	public function existsId($id){
		if($id){
			$marker = frameUms::_()->getTable('marker')->get('*', array('id' => $id), '', 'row');
			if(!empty($marker)){
				return true;
			}
		}
	...
	public function getById($id) {
		return $this->_afterGet(frameUms::_()->getTable('marker')->get('*', array('id' => $id), '', 'row'));
	}
	public function getMarkerByTitle($title) {
		return $this->_afterGet(frameUms::_()->getTable('marker')->get('*', array('title' => $title), '', 'row'));
	}
	public function _afterGet($marker, $widthMapData = false, $withoutIcons = false) {
		if(!empty($marker)) {
			...
			//if(uriUms::isHttps()) {
			//	$marker['description'] = uriUms::makeHttps($marker['description']);
			//}
			if($widthMapData && !empty($marker['map_id']))
				$marker['map'] = frameUms::_()->getModule('maps')->getModel()->getMapById($marker['map_id'], false);

			if($widthMapData && !empty($marker['map_id']))
				$marker['map'] = frameUms::_()->getModule('maps')->getModel()->getMapById($marker['map_id'], false);

			if(!empty($marker['id'])){
				$marker['marker_group_ids'] = frameUms::_()->getModule('marker')->getModel('marker_groups_relation')->getRelationsByMarkerId($marker['id']);
			}
		}
		return $marker;
	}
	...
	public function saveMarker($marker) {
		...
		if(!isset($marker['icon'])) {
			$marker['icon'] = 1;
		} elseif(!frameUms::_()->getModule('icons')->getModel()->iconExists($marker['icon'])) {
		...
		unset($marker['id']);
		$marker['create_date'] = date('Y-m-d H:i:s');
		$marker['params'] = utilsUms::serialize(array('titleLink' => $marker['titleLink']));
    ...
    /*public function updateMapMarkers($params, $mapId = null) {
        foreach($params as $id => $data) {
			...
			$data['params'] = utilsUms::serialize(array('titleLink' => $data['titleLink']));
			...
			$params[$id]['id'] = $newId;
			$params[$id]['params'] = utilsUms::unserialize($data['params']);
        }
        return $params;
    }*/
    /*public function updateMarker($marker){
        $insert = array(
			...
			'icon'              =>  $marker['icon']['id'],
			'params'			=>  utilsUms::serialize(array('titleLink' => $marker['titleLink']))
		);
    ...
    public function getMapMarkers($mapId, $withGroup = false, $userId = false) {
        ...
        $markers = frameUms::_()->getTable('marker')->orderBy('sort_order ASC')->get('*', $params);
		if(!empty($markers)) {
			$iconIds = array();
			...
			$usedIcons = frameUms::_()->getModule('icons')->getModel()->getIconsByIds( array_keys($iconIds) );
			foreach($markers as $i => $m) {
	...
	public function getMapMarkersIds($mapId) {
		return frameUms::_()->getTable('marker')->get('id', array('map_id' => $mapId), '', 'col');
	}
	public function getMarkersByIds($ids) {
		if(!is_array($ids))
			$ids = array( $ids );
		$ids = array_map('intval', $ids);
		$markers = frameUms::_()->getTable('marker')->get('*', array('additionalCondition' => 'id IN ('. implode(',', $ids). ')'));
		if(!empty($markers)) {
			foreach($markers as $i => $m) {
    ...
    public function removeMarker($markerId){
		...
		return frameUms::_()->getTable('marker')->delete(array('id' => $markerId));
    }
	public function removeList($ids) {
		$ids = array_map('intval', $ids);
		return frameUms::_()->getTable('marker')->delete(array('additionalCondition' => 'id IN ('. implode(',', $ids). ')'));
	}
    public function findAddress($params){
		...
		$apiDomain = frameUms::_()->getModule('maps')->getView()->getApiDomain();
    ...
    public function removeMarkersFromMap($mapId){
        return frameUms::_()->getTable('marker')->delete("`map_id`='".$mapId."'");
    }
    public function getAllMarkers($d = array(), $widthMapData = false) {
		if(isset($d['limitFrom']) && isset($d['limitTo']))
			frameUms::_()->getTable('marker')->limitFrom($d['limitFrom'])->limitTo($d['limitTo']);
		if(isset($d['orderBy']) && !empty($d['orderBy'])) {
			frameUms::_()->getTable('marker')->orderBy( $d['orderBy'] );
		}
        $markerList = frameUms::_()->getTable('marker')->get('*', $d);
        $iconsModel = frameUms::_()->getModule('icons')->getModel();
	...
	public function setMarkersToMap($addMarkerIds, $mapId) {
		...
		return frameUms::_()->getTable('marker')->update(array('map_id' => (int)$mapId), array('additionalCondition' => 'id IN ('. implode(',', $addMarkerIds). ')'));
	}
	public function getCount($d = array()) {
		return frameUms::_()->getTable('marker')->get('COUNT(*)', $d, '', 'one');
	}
	public function updatePos($d = array()) {
		$d['id'] = isset($d['id']) ? (int) $d['id'] : 0;
		if($d['id']) {
			return frameUms::_()->getTable('marker')->update(array(
				'coord_x' => $d['lat'],
				'coord_y' => $d['lng'],
			), array(
				'id' => $d['id'],
