...
class iconsModelUms extends modelUms {
   /* public static $tableObj;
    ...
    public function getIcons($params = array()) {
        ...
        $icons = frameUms::_()->getTable('icons')->get( $fields );
        if(empty($icons) ){
			return $icons ;
        }
        $iconsArr = array();
    ...
    public function setDefaultIcons(){
        ...
        foreach($icons as $icon){
			...
			$qItems[] = "('". $icon['title']. "','". $icon['description']. "','". $icon['img']. "', '"
					. (int)$size[0]. "', '". (int)$size[1]. "', 1)";
		}
		$query = "insert into `@__icons` (`title`,`description`,`path`, `width`, `height`, `is_def`) VALUES ". implode(",",$qItems);
		dbUms::query($query);
